...
// Hexagonal lattice for MATTER (crystalline grid)
const HEXES = (() => {
...
// Circuit traces for INTELLIGENCE (right-angle paths)
...
// Particle pool — distributed across pillar regions
const PARTICLES = [
  // LIFE — emerald, lower-left
  ...
  // MOTION — cyan, scattered along equator
...
function CivSphere() {
  ...
  useEffect(() => {
    ...
    const draw = (t) => {
      ...
      const cx = S / 2, cy = S / 2
      const r  = S * 0.415           // sphere radius
      const rot = t * 0.000095       // ultra-slow surface rotation

      ctx.clearRect(0, 0, S, S)

      // ── 1. OUTER GLOW — contained within canvas (no square bleed) ───
      // Arc radius r*1.15 = 0.477*S, canvas edge is 0.5*S — glow stays circular
      ...
      // Subtle warm bleed from energy region (kept within canvas)
      ...
      // Subtle cool bleed from life region (kept within canvas)
      ...
      // ── 2. SPHERE SURFACE (everything below is clipped to the disc) ───
      ctx.save()
      ctx.beginPath(); ctx.arc(cx, cy, r, 0, PI2); ctx.clip()

      // Base: deep dark with top-left ambient
      ...
      // Pillar region centres (slowly rotate with the sphere)
      ...
      // ── LIFE — organic emerald/teal, lower-left ───────────────────────
      ;[
      ...
      // ── ENERGY — radiant amber/gold, upper-right ──────────────────────
      ;[
      ...
      // Electric blue accent within energy
      {
      ...
      // ── MATTER — crystalline purple/silver, left ──────────────────────
      ;[
      ...
      HEXES.forEach(({ nx, ny }) => {
        ...
        // Crystal atom at vertex intersections
      ...
      // ── MOTION — orbital trails, cyan/blue, diffuse across equator ───
      {
      ...
      // ── CENTRE CONVERGENCE GLOW (all pillars merging) ─────────────────
      // Soft multi-colour bloom at sphere centre
      ;[
      ...
      // ── LIMB DARKENING (physics-accurate edge fade) ───────────────────
      ...
      // ── SPECULAR HIGHLIGHT (top-left, over rim) ───────────────────────
      ctx.save()
      ctx.beginPath(); ctx.arc(cx, cy, r, 0, PI2); ctx.clip()
      ...
      // ── RIM LIGHT (thin, inside sphere edge) ──────────────────────────
